...
model is moreover discarded at every congestion event, and what the window gains
while a congestion signal is in flight never surpasses what the reduction takes
back. A sender meeting congestion therefore comes away below the window that
provoked it.
...
congestion avoidance carries a round-trip time at least 1ms below low_rtt, the
point moves to that sample and to the window that accompanied it. The model is
fitted afresh on initialization and on every move. The 1ms threshold keeps
jitter from moving the point once the queue has drained and the round-trip time
no longer varies with the window.
...
if a > 0 and latest_rtt < a * cwnd + b:
  ...
  gain  = 1 / (1 + MAX_SLOPE_RATIO) * (1 - w_ref / cwnd)
  if cwnd < cwnd_prior / beta:
    gain = min(gain, pow(beta, -2 / 3) - 1)
...
in hand, the path has room the window is not using. Each round trip takes
1 / (1 + MAX_SLOPE_RATIO) of that room, three fifths.

That share is what the delay in the feedback affords. The path answers a round
trip later, so the gain is applied again before the answer arrives, and summed
over such a run it comes to no more than 1 / MAX_SLOPE_RATIO of the room first
observed. That is the margin the flattening created, so the run carries the
window no further than the fitted slope would have put it.

Below cwnd_prior / beta the gain is capped, so that the window grows over a
round trip by no more than pow(beta, -2/3). {{yield}} shows what that bound is
for.
...
can raise the congestion window but never lower it. Once the gain no longer
applies, the window holds, and once CUBIC's curve catches up, the increase is
handled by CUBIC.
...
Call the bar the window at which the path signals congestion. What has to hold
is that the sender comes away below it. ABBA caps the gain to that end: below a
window of cwnd_prior / beta, no round trip grows the window by more than
pow(beta, -2/3).

When congestion is signalled, the window stands at or above the bar.

With the bar below cwnd_epoch, the sender is over it from the outset, so the
signal is caused at once and reported a round trip later. Reaching cwnd_prior
would take a factor of 1 / beta, and the cap allows only pow(beta, -2/3) in that
round trip, so the signal is reported below cwnd_prior and the epoch that
follows stands at pow(beta, 1/3) of the one before it. Each such event shrinks
the epoch, and once it has fallen below the bar, the bar lies between it and
cwnd_prior: cwnd_prior is the window the signal was reported at.

With the bar between cwnd_epoch and cwnd_prior, the sender reaches the bar while
the cap is in force, and a round trip later, when the acknowledgement reports
the signal, the window stands at no more than

    bar * pow(beta, -2/3)

still below cwnd_prior / beta, so the cap held throughout. Reducing by beta
leaves

    bar * pow(beta, 1/3)

so the sender comes away under the bar for any beta short of one.

The cap governs the accelerated gain alone. CUBIC's own increase is not touched
by it, and where CUBIC sets the larger window that window stands, as it would
without ABBA.
